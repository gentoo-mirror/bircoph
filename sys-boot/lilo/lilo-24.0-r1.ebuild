...
lilocheck() {
	...
	# If password is set to "" in the config file, lilo will
	# interactively request to enter it, see # 497288.
	# Lilo may or may not ask for password depending on multiple
	# complicated factors:
	# - lilo.conf was modified after last lilo -p run
	# - lilo binary was changed (e.g. version bump or rebuild with
	#   different compiler).
	# To be on the safe side, remind user to reinstall lilo sector
	# each time and don't run dolilo automatically.
	if grep -q "^[[:space:]]*password[[:space:]]*=[[:space:]]*\"\"" /etc/lilo.conf
	then
		ewarn
		ewarn "Yes, but your /etc/lilo.conf requires interactive password."
